...
local custom_conf = {
  files = function()
    ...
    local toggle_dotfiles = function()
      ...
      local dotfiles_filter = show_dotfiles and function(_)
        return true
      ...
      extra.pickers.spellsuggest(local_opts, {
        source = {
          name = ('Spell suggestions (%s) for %s'):format(
            string.upper(vim.api.nvim_get_option_value('spelllang', { buf = 0 })),
  ...
  git = function()
    ...
    local format_summary_string = function(data)
      vim.b[data.buf].minigit_summary_string = vim.b[data.buf].minigit_summary.head_name
    end
    ...
    aug.add('User', { pattern = 'MiniGitUpdated', callback = format_summary_string })
    aug.add('User', { pattern = 'MiniGitCommandSplit', callback = align_blame })
