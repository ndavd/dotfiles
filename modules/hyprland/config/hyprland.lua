local ok, vars = pcall(require, 'vars')
if not ok then
  hl.notification.create({
    text = 'Missing vars.lua; should be handled by Nix',
    timeout = 10000,
    icon = 3,
  })
end
...
if vars.secondary_monitor then
  hl.monitor({
    output = vars.secondary_monitor.output,
    mode = monitor_mode(
      vars.secondary_monitor.width,
      vars.secondary_monitor.height,
...
for i = 1, 10 do
  ...
  hl.workspace_rule({
    ...
    monitor = vars.secondary_monitor and i % 2 == 0 and vars.secondary_monitor.output
      or vars.primary_monitor.output,
  })
...
hl.bind(keys(mainMod, 'return'), hl.dsp.exec_cmd(vars.terminal))
...
hl.bind(keys(mainMod, 'b'), hl.dsp.exec_cmd(vars.browser))
hl.bind(
  keys(mainMod, 'p'),
  hl.dsp.exec_cmd(
    ('notify-send "mpv" "Playing $(wl-paste)" && mpv --ytdl-raw-options=cookies-from-browser=%s "$(wl-paste)"'):format(
      vars.browser
    )
  )
)
